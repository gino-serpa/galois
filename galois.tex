...
are $0$ the solution is called non-trivial; otherwise, it is called trivial.
...
We seek elements $x_1, \ldots , x_n$ not all $0$ such that $L_1 = L_2 = \ldots = L_m = 0$.

If $a_{ij} = 0$ for each $i$ and $j$, then any choice of $x_i , \ldots , x_r$, will serve as
a solution. 
...
\vdots \\
Lm - a_{m1} a_{11}^{-1} L_1 & = 0
\end{align*}

For, if $x_1,\ldots ,x_n$ is a solution of these latter equations then, since
$L_1 = 0$, the second term in each of the remaining equations is $0$ and,
hence, $L_1 = L_2 = . . . = L_m = 0$. 
...
in the form $x_j a_{ij} = 0, j = 1,2,\ldots, n$, the above theorem would still
...
that $ x_1 A_1 + x_2 A_2 + \ldots + x_n A_n = 0$. 
...
\begin{equation}
a_1 A_1 + a_2 A_2+ \ldots + a_r A_r + a_{r+j} A{r+j} = 0
\end{equation}
...
\begin{equation}
x_1 B_1 + x_2 B_2 + \ldots + x_t B_t = 0
\end{equation}
...
A solution to the equations $\sum_{i=1}^t x_j a_{ij} = 0$, $i = 1, \ldots, r$, always exists by Theorem 1.
\end{proof}
...
Solving for $A$ in terms of $A,...,A,$, exhibits $A,, . . . ,An$ as a generating system.
